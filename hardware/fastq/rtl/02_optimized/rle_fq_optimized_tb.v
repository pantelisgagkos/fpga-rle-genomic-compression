...
initial begin
    ...
    fd = $fopen("/home/pantelis/Documents/test_head.fq", "r");
...
always @(posedge clk) begin 
    if (out_valid) begin
        if (out_count > 32'd5) begin
            if (out_quality) 
            $display("RLE QUALITY: %0d%c", out_count, out_char);
            else
            $display("RLE SEQUENCE: %0d%c", out_count, out_char);
            
        end
        else begin
            if (out_quality) begin
                for (i = 0; i < out_count; i = i +1) begin
                $display("RLE QUALITY: %c", out_char);
                end
            end
            else 
            for (i = 0; i < out_count; i = i +1) begin
                $display("RLE SEQUENCE: %c", out_char);
            end
